 ...
 * lexer will be presented with a purified source that only contains data
 * division entries.
 * <p>
...
public class CopybookCleaner {
	...
	public Result clean(Reader reader) {
		...
		try (LineNumberReader r = new LineNumberReader(reader)) {
			...
			while ((l = r.readLine()) != null) {
				...
				if (cl.isBlank() || isComment(cl) || isDebug(cl)) {
					continue;
				}
    ...
    class Result {
		...
		/**
		 * For each clean line, keep track of the original line nummber (before
		 * cleaning)
		 */
		final List<Integer> cleanLineNumbers = new ArrayList<>();
		...
		public String toString() {
			...
			for (int i = 0; i < cleanLines.size(); i++) {
				sb.append(String.format("%03d", cleanLineNumbers.get(i)));
				sb.append(" ");
